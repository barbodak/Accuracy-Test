<script lang="ts">
	...
	onMount(async () => {
		try {
			...
			answers = quiz.answers.map((x: any) => (x === 1 ? 'S' : x === 2 ? 'D' : '0'));
			const quizDuration = 10 * 60; // 10 minutes
			const qdate = new Date(quiz.quiz_time);
			const now = new Date();
			const timeElapsed = Math.floor((now.valueOf() - qdate.valueOf()) / 1000);
	...
	<nav class="container mx-auto px-4 sm:px-6 lg:px-8">
		<div class="flex items-center justify-between h-16">
			<div class="flex items-center">
				<span class="text-xl font-bold text-slate-800">Accuracy Test</span>
			</div>
...
{#if isOverlayOpen}
	<Overlay on:close={() => (isOverlayOpen = false)} isTransparent={true} canBeExited={false}>
		<div class="text-center p-4">
			<h3 class="text-xl font-bold text-white mb-2">End the Test?</h3>
			<p class="text-slate-300 mb-6">Are you sure you want to end this test?</p>
